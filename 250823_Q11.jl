...
if HX_E.isCounterFlow == true
    ...
    eva_Fld = [
        [
            ...
            T_Fld[N+2-i] ~ T_Fld[N+1-i] - Q_f[N+1-i] / (HX_mdot_Fld * HX_Fld_Cp),
        ]
        for i in 1:N
    ]
else
    ...
    eva_Fld = [
        [
            ...
            T_Fld[i+1] ~ T_Fld[i] - Q_f[i] / (HX_mdot_Fld * HX_Fld_Cp),
        ]
        for i in 1:N
    ]
end
...
# Sys
@mtkbuild sys = ODESystem(eva_model, t)
tspan = (0.0, 15.0)
